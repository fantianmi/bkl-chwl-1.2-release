package com.unionpay.acp.payanother;
...
public class Form10_6_2 extends DemoBase {
	 ...
	 * @return
	 */
	public static Map<String, Object> setFormDate(Cash order) {
		Map<String, Object> contentData = new HashMap<String, Object>();
		String merId = MainConfig.getUnionpayMerid();
		String txnTime = TimeUtil.getNowDateTime4NotSplit();// --订单发送时间
		String orderId = order.getOrderId();// --商户订单号
		String bank_account=order.getCard();
		String txnAmtI=String.valueOf((int)(order.getAmount()*100));
		...
		// 非绑定类交易时需上送卡号
		contentData.put("accNo", bank_account);// M
	...
	public static Map<String, String> doSubmitWithdraw(Cash order) {
		/**
		 * 参数初始化
		...
		String requestBackUrl = SDKConfig.getConfig()
				.getBackRequestUrl();
		Map<String, String> resmap = submitDate(setFormDate(order),requestBackUrl);
		return resmap;
	}
	
	public static void main(String args[]){
		Cash order=new Cash();
		order.setBank_number("6216261000000000018");
		Double amount=200d;
		order.setAmount(amount);
		order.setOrderId(TimeUtil.getNowDateTime4NotSplit());
		Map<String, String> resmap=doSubmitWithdraw(order);
		System.out.println(resmap.get("flag"));
	}
